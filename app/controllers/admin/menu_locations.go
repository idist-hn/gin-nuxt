...
func ListMenuLocations(c *gin.Context) {
	data := bson.M{}
	var err error
	entry := collections.MenuLocation{}
	entries := collections.MenuLocations{}
...
func CreateMenuLocation(c *gin.Context) {
	user := c.MustGet("user").(collections.User)
	data := bson.M{}
	var err error
	entry := collections.MenuLocation{}
	exist := collections.MenuLocation{}
	...
	if err = exist.First(filter); err != nil && err != mongo.ErrNoDocuments {
		...
		controllers.ResponseError(c, http.StatusConflict, "Đã tồn tại vị trí này.", nil)
	}
...
func GetMenuLocation(c *gin.Context) {
	data := bson.M{}
	var err error
	entry := collections.MenuLocation{}
...
func UpdateMenuLocation(c *gin.Context) {
	user := c.MustGet("user").(collections.User)
	data := bson.M{}
	var err error
	var entry, entryUpdate collections.MenuLocation
	...
	// Kiểm tra xem có tồn tại vị trí này không
	if err = entry.First(filter); err != nil && err != mongo.ErrNoDocuments {
		controllers.ResponseError(c, http.StatusInternalServerError, "Truy vấn dữ liệu lỗi", err)
	...
	// Kiểm tra xem alias có bị trùng không
	filter = bson.M{
		"deleted_at": nil,
		"alias":      entryUpdate.Alias,
	}
	if err = entry.First(filter); err != nil && !errors.Is(err, mongo.ErrNoDocuments) {
		controllers.ResponseError(c, http.StatusInternalServerError, "Truy vấn dữ liệu lỗi", err)
	} else if errors.Is(err, mongo.ErrNoDocuments) {
	} else if entry.ID != entryUpdate.ID {

		fmt.Println(entry.ID, entryUpdate.ID)
		controllers.ResponseError(c, http.StatusConflict, "Đã tồn tại vị trí này.", nil)
	}

	entryUpdate.UpdatedBy = user.ID
...
func DeleteMenuLocation(c *gin.Context) {
	data := bson.M{}
	var err error
	entry := collections.MenuLocation{}
