...
func ListCategories(c *gin.Context) {
	...
	for i := 0; i < len(entries); i++ {
		entries[i].Preload("count_articles")
	}
...
func CreateCategory(c *gin.Context) {
	...
	if err = exist.First(filter); err != nil && err != mongo.ErrNoDocuments {
		...
		controllers.ResponseError(c, http.StatusConflict, "Đường dẫn đã tồn tại", err)
	}
...
func UpdateCategory(c *gin.Context) {
	...
	// Kieemr tra xem slug đã tồn tại chưa
	filter = bson.M{
		"deleted_at": nil,
		"slug":       entryUpdate.Slug,
		"_id":        bson.M{"$ne": entryUpdate.ID},
	}
