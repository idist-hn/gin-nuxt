...
func ListTemplates(c *gin.Context) {
	data := bson.M{}
	var err error
	entry := collections.Template{}
	entries := collections.Templates{}

	pagination := BindRequestTable(c, "updated_at")
	filter := pagination.CustomFilters(bson.M{})
	if pagination.Search != "" {
		filter["$or"] = bson.A{
			bson.M{"name": bson.M{"$regex": pagination.Search, "$options": "i"}},
			bson.M{"slug": bson.M{"$regex": pagination.Search, "$options": "i"}},
		}
	}
...
func UpdateTemplate(c *gin.Context) {
	data := bson.M{}
	var err error
	var entry, entryUpdate collections.Template
