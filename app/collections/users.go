...
type User struct {
	...
	Roles        Roles          `bson:"-" json:"roles"`
	Permissions  []Permission   `bson:"-" json:"permissions"`
	OldPasswords []UserPassword `bson:"old_passwords" json:"-"`
}
	...
	for _, property := range properties {
		...
		if property == "department" {
			...
			go func() {
				...
				_ = department.First(bson.M{"_id": u.DepartmentId, "deleted_at": nil})
				department.Content = ""
				department.Description = ""
				department.PcDepartmentStyles = nil
				department.MobileDepartmentStyles = nil
		...
		if property == "roles" {
			...
			go func() {
				defer wg.Done()
				var role Role
				u.Roles, _ = role.Find(bson.M{"_id": bson.M{"$in": u.RoleIds}, "deleted_at": nil}, options.Find())
			}()
		}
		if property == "roles.permissions" {
			wg.Add(1)
			go func() {
				defer wg.Done()
				permission_ids := make([]primitive.ObjectID, 0)
				permission := Permission{}
				u.Preload("roles")
				for _, role := range u.Roles {
					permission_ids = append(permission_ids, role.PermissionIds...)

				}
				u.Permissions, _ = permission.Find(bson.M{"_id": bson.M{"$in": permission_ids}}, options.Find())
			}()
		}
		if property == "old_passwords" {
			wg.Add(1)
			go func() {
				defer wg.Done()
				var oldPassword UserPassword
				u.OldPasswords, _ = oldPassword.Find(bson.M{"user_id": u.ID}, options.Find().SetSort(bson.M{"created_at": -1}).SetLimit(5))
			}()
		}
	}
	wg.Wait()
...
func (u *User) HasPermission(Subject, Action string) bool {
...
func (u *User) ComparePassword(password string) bool {
	...
	fmt.Println("u.Password", u.Password, "password", password)
...
func (u *User) ComparePasswordHistory(password string) bool {

	u.Preload("old_passwords")

	for _, entry := range u.OldPasswords {
