<template>
  <a-card class="department-attributes" title="Định dạng trên di động">
    <a-row v-for="(attribute, index) in attributes" :gutter="16" :key="attribute.id" class="mb-10">
      ...
      <a-col :md="2">
        <a-form-item v-if="index == 0" prop="action" name="action" label="1">
...
export default {
  ...
  watch: {
    attributes: {
      handler: function () {
        this.value.mobile_department_styles = this.attributes;
      },
      deep: true,
    },
  },
  ...
  created() {
    this.attributes = this.value.mobile_department_styles ?? [];
  },
};
</script>
<style lang="scss">
