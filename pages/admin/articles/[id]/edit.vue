<template>
  <a-page-header title="Chỉnh sửa Bài viết" @back="() => routerBack()"/>
  <a-form v-if="!loading" ref="FormData" layout="vertical" :rules="rules" :model="entry">
    <a-row :gutter="16">
      ...
      <a-col :xxl="6" :lg="6" :md="6" :sm="24">
        ...
        <a-card class="mb-10">
          <a-space :size="6">
            <a-button v-if="entry.status !=='published'" :loading="loading" :disabled="loading" type="primary"
                      html-type="submit"
                      @click.prevent="submitForm(entry.status)">
              Cập nhật
            </a-button>
            <a-button :loading="loading" :disabled="loading" type="success" html-type="submit"
                      @click.prevent="submitForm('published')">
              Xuất bản
            </a-button>
            <a-popconfirm placement="top" ok-text="Đồng ý" cancel-text="Không" @confirm="routerBack">
              <template v-slot:title>
                <p>Huỷ cập nhật bài viết?</p>
                <p>Bạn xác nhận huỷ chỉnh sửa bài viết này...</p>
              </template>
  ...
  <a-card v-else class="d-flex align-items-center justify-content-center">
    <a-spin/>
  </a-card>
</template>
<script>
...
export default {
  ...
  methods: {
    ...
    async getData() {
      this.loading = true
      let response = await useArticleStore().GetEntry({id: this.$route.params.id})
    ...
    async submitForm(status) {
      try {
        ...
        let response = await useArticleStore().UpdateEntry(this.entry);
